...
The v2 SSH handoff documents are retained as historical/fallback references. They no longer define the product boundary. Phase 4 adds exact installed-help-backed monitoring, wireless/USB inventory, forensics, replay, ADS-B/AIS, Bluetooth discovery, MQTT/relay, Modbus-read, smartcard/YubiKey, camera stream, and NTRIP workflows. The remaining CAN transmit/configure, Modbus write, USB power/attach, wireless monitor-mode, and cellular raw-command gaps have concrete runtime/topology/rollback obstacles documented in [PHASE4-OPERATOR.md](PHASE4-OPERATOR.md). Missing hardware or executable payloads remain honest blockers; the action class by itself is not one.
