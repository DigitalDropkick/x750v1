...
| Video / UVC camera | Each selectable primary node must resolve through sysfs to `uvcvideo`, USB interface class `0e`, a valid VID:PID, and sysfs index `0`. The action separately confirms `Video Capture` with bounded `v4l2-ctl`. |
| RTL-SDR | Each selectable reviewed `0bda:2832` or `0bda:2838` device must have a USB serial that passes the strict character/length policy and no attached interface driver. |
...
- A compatible-looking RTL-SDR with a different VID:PID remains unreviewed and cannot enable the receiver. Multiple approved dongles are individually selectable by their validated server-derived serial; the worker revalidates the chosen identity before opening it.
- A UVC device with several nodes contributes only nodes reporting sysfs index `0`. Multiple reviewed primary nodes are individually selectable by their server-derived `/dev/videoN` identity; the worker revalidates V4L2 capture capability, USB identity, and exclusive availability before use.
