...
The v2.1 migration currently provides practical Nmap 7.91, tcpdump 4.9.3, iperf3 3.11, RTL-433, UVC still, non-EC25 serial, GPS/GNSS, and ADB 1.0.32 controls. These use validated target/interface/device selection, native options, cancellation, resource locking, review/confirmation where consequential, authenticated artifacts, and sealed file inputs. Existing v2.0 behavior remains intact while the other tool families are migrated incrementally.
...
Version 2.1 is currently a local feature-branch build. Its source and target-Lua syntax checks have passed, but it has not been deployed or accepted on the production router. The evidence above describes the preserved v2.0 baseline, not v2.1 production status.
...
See [docs/OPERATOR-MODE.md](docs/OPERATOR-MODE.md), [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md), [docs/SECURITY.md](docs/SECURITY.md), [docs/ANDROID-ADB.md](docs/ANDROID-ADB.md), [docs/DEVICE-IDENTITY.md](docs/DEVICE-IDENTITY.md), [docs/SSH-TOOL-HANDOFFS.md](docs/SSH-TOOL-HANDOFFS.md), [docs/BRAND-SYSTEM.md](docs/BRAND-SYSTEM.md), [docs/PACKET-CAPTURE.md](docs/PACKET-CAPTURE.md), [docs/RTL433-RECEIVE.md](docs/RTL433-RECEIVE.md), [docs/CAMERA-SNAPSHOT.md](docs/CAMERA-SNAPSHOT.md), [docs/GPS-GNSS-SNAPSHOT.md](docs/GPS-GNSS-SNAPSHOT.md), and [docs/CAN-PASSIVE-CAPTURE.md](docs/CAN-PASSIVE-CAPTURE.md).
...
- Version 2.1 is not deployed. The production appliance remains on the accepted v2.0 baseline until Addam separately authorizes deployment.
- Operator Mode currently covers Nmap, tcpdump, iperf3, RTL-433, UVC still capture, non-EC25 serial, GPS/GNSS, Android ADB, and Apple normal/recovery/DFU/restore workflows. The remaining manifests retain their v2 fixed/identity/handoff behavior until each exact installed tool is audited and migrated; `ACTION`, `SECURITY`, or `DISRUPTIVE` classification is not itself a reason to keep one disabled.
...
- Authenticated input staging now reserves one DDK-owned extroot path, validates name/type/size/free-space, atomically seals the result, computes SHA-256, and applies one-hour reservation/24-hour sealed retention. ADB and Apple recovery/restore actions bind and revalidate sealed IDs; programming and storage consumers remain unavailable until their exact actions do the same.
