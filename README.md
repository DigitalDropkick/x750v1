...
Production-oriented LuCI control dashboard for the GL.iNet GL-X750 field appliance. Version 1.9.0 completes Burn One with hardware-gated GPS/GNSS and passive CAN snapshots. GPS uses the already-installed `gpsdecode`, requires one precisely attributed external USB receiver, rejects every EC25-AF modem port, and deletes raw NMEA after rendering whitelisted transient fields. CAN requires one already-up physical `canN` plus `/usr/bin/candump`, never configures the interface, and never transmits. The current router has neither external receiver nor CAN interface, and its `canutils` package record contains no `candump`; the UI reports those exact conditions and disables both actions without installing or replacing anything. Camera stills, RTL-433, live health, inventory, branded responsive UI, bounded Nmap and LAN metadata workflows, cellular and serial attribution, swap persistence, and `/ddk` remain intact—without adding a daemon or listener.
