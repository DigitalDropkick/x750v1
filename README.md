...
Production-oriented LuCI control dashboard for the GL.iNet GL-X750 field appliance. Version 1.7.0 adds a hardware-gated, single-frame UVC camera workflow using the already-installed `fswebcam` and V4L2 tools. With no camera attached, the action remains visibly `HARDWARE REQUIRED` and is rejected before job creation. RTL-433, live health, inventory, branded responsive UI, bounded Nmap and LAN metadata workflows, cellular and serial attribution, swap persistence, and `/ddk` remain intact—without adding a daemon or listener.
...
See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md), [docs/BRAND-SYSTEM.md](docs/BRAND-SYSTEM.md), [docs/PACKET-CAPTURE.md](docs/PACKET-CAPTURE.md), [docs/RTL433-RECEIVE.md](docs/RTL433-RECEIVE.md), [docs/CAMERA-SNAPSHOT.md](docs/CAMERA-SNAPSHOT.md), and [docs/SECURITY.md](docs/SECURITY.md).
...
Verification covers identity, extroot, swap, installed files, locally served brand assets, Lua/shell/JSON syntax, live APIs, all INFO actions, injection rejection, generic-PID rejection, traversal rejection, asynchronous jobs, Nmap proofs, non-promiscuous LAN metadata capture and cancellation, RTL-433 and UVC camera hardware/no-device gating, authenticated camera-artifact ACL isolation, the cellular privacy/read-only snapshot, system reports, GL.iNet UI, LuCI, dashboard route, Tailscale, protected configuration hashes including `rtl_tcp`, `mjpg-streamer`, and Motion, listener/worker absence, memory, disk, and recent errors.
