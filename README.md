...
Production-oriented LuCI control dashboard for the GL.iNet GL-X750 field appliance. Version 1.8.0 adds a hardware-gated, receive-only GPS/GNSS position snapshot using the already-installed `gpsdecode`. It requires one precisely attributed external USB receiver, refuses every EC25-AF modem port, exposes no device or serial controls to the browser, and removes raw NMEA after producing whitelisted transient fields. No receiver is currently attached, so the action remains visibly `HARDWARE REQUIRED` and is rejected before job creation. Camera stills, RTL-433, live health, inventory, branded responsive UI, bounded Nmap and LAN metadata workflows, cellular and serial attribution, swap persistence, and `/ddk` remain intact—without adding a daemon or listener.
...
See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md), [docs/BRAND-SYSTEM.md](docs/BRAND-SYSTEM.md), [docs/PACKET-CAPTURE.md](docs/PACKET-CAPTURE.md), [docs/RTL433-RECEIVE.md](docs/RTL433-RECEIVE.md), [docs/CAMERA-SNAPSHOT.md](docs/CAMERA-SNAPSHOT.md), [docs/GPS-GNSS-SNAPSHOT.md](docs/GPS-GNSS-SNAPSHOT.md), and [docs/SECURITY.md](docs/SECURITY.md).
